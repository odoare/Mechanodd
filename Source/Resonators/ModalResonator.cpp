...
void ModalResonator::recomputeModes()
{
    ...
    const int mmax = kmax;
    const int nmax = juce::jmin (maxCandidates / juce::jmax (1, mmax),
                                 (int) std::ceil (kmax * aspect) + 2);

    int count = 0;
    for (int m = 1; m <= mmax && count < maxCandidates; ++m)
        for (int n = 1; n <= nmax && count < maxCandidates; ++n)
            candidates[(size_t) count++] = { modeFrequency (m, n, fundamental, aspect), m, n };
    ...
    for (int i = 0; i < count && activeModes < numModes; ++i)
    {
        ...
        ampGeom[(size_t) activeModes]  = modeShape (c.m, c.n, inX, inY) * modeShape (c.m, c.n, outX, outY);
...
float ModalResonator::processSample (float input)
{
    ...
    // mode is costly, so only do it when the gate has moved appreciably.
    const float g = advanceGate();
    if (std::abs (g - gateAtUpdate) > 0.02f)
    {
